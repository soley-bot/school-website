  ...
  const handleBasicInfoUpdate = async (updatedInfo: Partial<Program>) => {
    try {
      const { error } = await supabase
        .from('program_pages')
        .upsert({ ...program, ...updatedInfo } as Record<string, unknown>);
      if (error) throw error;
  ...
  const handleContentUpdate = async (updatedContent: ProgramContent) => {
    try {
      const { error } = await supabase
        ...
        .upsert({ ...updatedContent, program_id: programId } as Record<string, unknown>);
      if (error) throw error;
  ...
  const handleScheduleUpdate = async (updatedSchedule: ProgramSchedule) => {
    try {
      const { error } = await supabase
        ...
        .upsert({ ...updatedSchedule, program_id: programId } as Record<string, unknown>);
      if (error) throw error;
